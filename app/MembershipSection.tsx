...
const MembershipSection = () => {
  ...
  <div className="absolute top-2 left-2 w-full h-full -z-10">
    <Image src={mbg} alt={''} className="object-cover w-[200px] h-[300px]" />
  </div>
  ...
  <div className="inline-flex items-center p-1 bg-black rounded-full mb-8 z-10 relative">
    <button
      className={`px-6 py-2 rounded-full transition-colors ${
        selectedPlan === 'standard' ? 'bg-yellow-400 text-black' : 'text-white hover:text-yellow-400'
      }`}
    ...
    <button
      className={`px-6 py-2 rounded-full transition-colors flex items-center gap-2 ${
        selectedPlan === 'pro' ? 'bg-yellow-400 text-black' : 'text-white hover:text-yellow-400'
      }`}
  ...
  <div className="mb-8 z-10 relative">
    ...
    <p className="text-gray-600 mt-2">{plans[selectedPlan].description}</p>
  </div>
